  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!testRes.ok || !testJson.success) {
        throw new Error(testJson.error || "Failed to connect to Gmail IMAP server")
      }
      ...
      <div className="max-w-4xl mx-auto space-y-8 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-full border border-blue-500/20 backdrop-blur-sm">
            ...
            <span className="text-sm font-medium text-blue-300">Gmail Integration</span>
          </div>
          <h1 className="text-4xl font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent">
            Gmail Settings
          </h1>
          <p className="text-slate-400 max-w-2xl mx-auto">
            Connect your Gmail account for automatic transaction monitoring and seamless email processing
          </p>
        </motion.div>
            ...
            <CardContent className="p-6">
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-4 flex-1">
                  <h3 className="text-lg font-semibold text-blue-300">How to get Gmail App Password</h3>
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-3">
                      <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg border border-slate-700/50">
                        ...
                        <span className="text-sm text-slate-300">Enable 2-Step Verification</span>
                      </div>
                      <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg border border-slate-700/50">
                        <div className="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                          2
                        </div>
                        <span className="text-sm text-slate-300">Go to App Passwords</span>
                      </div>
                      <div className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg border border-slate-700/50">
                        <div className="w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                          3
                        </div>
                        <span className="text-sm text-slate-300">Generate new password</span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <Button
            ...
            <CardHeader className="relative">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl text-white">Gmail Configuration</CardTitle>
            ...
            <CardContent className="space-y-6 relative">
              ...
              <div className="grid gap-6">
                ...
                <motion.div
                  ...
                  className="space-y-3"
                >
                  <Label htmlFor="email_user" className="text-slate-300 font-medium">
                    Gmail Address
                  </Label>
                  <div className="relative group">
                    ...
                    {!isEditing && formData.email_user && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => copyToClipboard(formData.email_user, "Email")}
                  ...
                  {isEditing && !validateGmailEmail(formData.email_user) && formData.email_user && (
                ...
                <motion.div
                  ...
                  className="space-y-3"
                >
                  <Label htmlFor="email_pass" className="text-slate-300 font-medium">
                    Gmail App Password
                  </Label>
                  <div className="relative group">
                  ...
                  {isEditing && (
                    <p className="text-xs text-slate-500">
                      Enter the 16-character app password from your Google Account settings
                    </p>
                  )}
                </motion.div>
                  ...
                  >
                    <div className="space-y-3">
                      ...
                      <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
                        <p className="text-white font-mono text-sm">imap.gmail.com</p>
                        <p className="text-xs text-slate-400 mt-1">Auto-configured for Gmail</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <Label className="text-slate-300 font-medium flex items-center gap-2">
                      ...
                      <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
                        <p className="text-white font-mono text-sm">993 (SSL)</p>
                        <p className="text-xs text-slate-400 mt-1">Secure connection</p>
                      </div>
                    </div>
                  </motion.div>
                )}
              ...
              {!isEditing && settings && (
                ...
                >
                  <div className="flex items-center gap-3 text-green-400 mb-3">
                    ...
                    <span className="font-semibold text-lg">Gmail Connected Successfully</span>
                  </div>
                  <p className="text-sm text-green-300/80 leading-relaxed">
                    Your Gmail account is securely connected and configured for automatic transaction monitoring. The
                    system uses Gmail's IMAP server with SSL encryption to ensure your data remains protected.
                  </p>
                </motion.div>
              )}
            </CardContent>
