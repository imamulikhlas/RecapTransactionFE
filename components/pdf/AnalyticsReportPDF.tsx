  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <Text style={styles.subHeader}>Financial Summary Report</Text>
        ...
        <View style={styles.section}>
          ...
          {categoryData.map((cat, idx) => (
            <View key={idx} style={styles.row}>
              <Text>{cat.category} ({cat.count}x)</Text>
              <Text>{formatCurrency(cat.amount)} ({cat.percentage}%)</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.section}>
          ...
          {chartData.map((d, idx) => (
            <View key={idx} style={styles.row}>
              <Text>{d.period}</Text>
              <Text>
                Income: {formatCurrency(d.income)} | Expense: {formatCurrency(d.expense)}
              </Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.transactionList}>
            {transactions.map((tx, idx) => {
              ...
              const desc = tx.note_to_user || tx.description || tx.merchant || 'No description'
              return (
